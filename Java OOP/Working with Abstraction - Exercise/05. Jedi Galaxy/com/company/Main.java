...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        int[] dimestions = Arrays.stream(reader.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
        int x = dimestions[0];
        int y = dimestions[1];
        ...
        while (!command.equals("Let the Force be with you")) {
            ...
            int[] evil = Arrays.stream(reader.readLine().split(" ")).mapToInt(Integer::parseInt).toArray();
            int xE = evil[0];
            int yE = evil[1];
